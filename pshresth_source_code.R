...
# Model Building
  ...
  output$accurracy_rate <- renderValueBox({
    valueBox("Accuracy Rate", value = paste(round(predictions()[1] * 100, digits = 2), "%"), # accuracy rate
             icon = icon("bullseye"), color = "red")
  })
  
  output$goodness_of_fit <- renderValueBox({
    valueBox("Goodness of Fit", value = paste(round(predictions()[2] * 100, digits = 2), "%"), # goodness of fit
